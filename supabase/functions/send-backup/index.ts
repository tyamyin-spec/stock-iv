...
async function sendEmail(to: string[], subject: string, html: string, att: { filename: string; content: string }) {
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({ from, to, subject, html, attachments: [att] }),
  });
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const att = { filename: `stock-iv-backup_${stamp}.json`, content: base64(JSON.stringify(snapshot)) };
    const html =
      `<p>ไฟล์สำรองข้อมูลระบบจัดการสารน้ำ (Stock IV)</p>` +
      `<p>วันที่ ${now.toLocaleString('th-TH')} · รวม ${rowCount} แถว</p>` +
      `<p style="color:#888;font-size:12px">เก็บไฟล์นี้ไว้ในที่ปลอดภัย ใช้กู้คืนข้อมูลได้</p>`;
    await sendEmail(recipients, `[Stock IV] สำรองข้อมูล — ${now.toLocaleDateString('th-TH')}`, html, att);
